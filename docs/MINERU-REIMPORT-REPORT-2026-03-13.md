...
## 结论
- 本轮继续执行了 `home-ai` 高图文第一批剩余文档，以及跨库最高优先级的 `政企版`、`视联百川客户端`。
- 实际补传并验证通过 `15` 份文档，`0` 失败。
- 验证脚本结果：
  - `total = 15`
  - `passed = 15`
  - `failed = 0`
...
- 全量验证结果：
  - [/Users/tony/Documents/GitHub/kb-cs-assistant/tmp/mineru-verify-20260313/mineru-reimport-verify-2026-03-12T16-22-07-064Z.json](/Users/tony/Documents/GitHub/kb-cs-assistant/tmp/mineru-verify-20260313/mineru-reimport-verify-2026-03-12T16-22-07-064Z.json)

## 全面验证

### 1. 新 collection 有效
- `15/15` 文档的新 collection 均能正常读取 detail
- 所有文档都满足：
  - `rawTextLength > 0`
  - `indexAmount > 0`
  - `detailStatus = chunk`
...
### 3. 旧 collection 删除有效
- 有旧 collection 的 `12` 份文档，删除验证全部通过
- 结果均返回：
  - `unExistCollection`
...
- `home-ai` 第一批高图文文档已经基本清完。
- 下一步更值得做的是：
  1. 继续 `home-ai` 第二批中等图文文档
  2. 再评估 `device-shop` 那两份 `2` 图文档是否值得补传
  3. 处理内部支持库后，再单独看 `视频AI算法舱`
